...
const DT: Record<string, Record<Lang, string>> = {
  ...
  regen_body: { fr: "Vos QR déjà imprimés continueront de fonctionner — le lien ne change jamais. Régénérer crée juste un nouveau fichier image.", ar: 'كودات QR المطبوعة ستستمر في العمل — الرابط لا يتغير أبداً.', en: 'Already printed QR codes will keep working — the link never changes. Regenerate just creates a new image file.' },
  ...
  tip_qr:     { fr: '💡 Le lien du QR ne change jamais. Régénérer crée juste un nouveau fichier.', ar: '💡 رابط QR لا يتغير أبداً.', en: '💡 The QR link never changes. Regenerate just creates a new file.' },
...
export default function DashboardClient({ business, form, submissions, userEmail }:
  ...
  async function saveQ() {
    ...
    const categories = questions.map((q,i)=>({id:String(i+1),label_fr:q.label_fr,label_ar:q.label_ar||q.label_fr,label_en:q.label_en||q.label_fr}))
    await supabase.from('feedback_forms').update({categories}).eq('id',form.id)
    setQuestions(categories); setQuestionsDirty(false); setQuestionsSaving(false)
  }
  ...
  const tabLabel = (id: string) => {
    const map: Record<string,string> = {overview:dt('overview'),reviews:`${dt('reviews')} (${submissions.length})`,questions:dt('questions'),qr:dt('qr'),settings:dt('settings')}
    return map[id] || id
  }
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .sb-av{width:32px;height:32px;border-radius:9px;background:linear-gradient(135deg,#028090,#00b4c8);display:flex;align-items:center;justify-content:center;font-family:'Cabinet Grotesk',sans-serif;font-weight:900;font-size:12px;color:#fff;flex-shrink:0}
        ...
        .tlsw{display:flex;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.07);border-radius:8px;padding:2px;gap:1px}
        .tlb{display:flex;align-items:center;gap:3px;padding:3px 7px;border-radius:6px;border:none;cursor:pointer;font-size:10px;font-weight:700;fontFamily:'Instrument Sans',sans-serif;background:transparent;color:#4a5a72;transition:all .15s;white-space:nowrap}
        .tlb.on{background:#028090;color:#fff}
        ...
        /* MOBILE */
        .mobile-topbar{display:none;padding:0 16px;height:54px;border-bottom:1px solid rgba(255,255,255,.06);align-items:center;justify-content:space-between;background:#07101f;position:sticky;top:0;z-index:100}
        .mobile-nav{display:none;position:fixed;top:54px;left:0;right:0;background:rgba(7,16,31,.97);backdrop-filter:blur(20px);border-bottom:1px solid rgba(255,255,255,.08);padding:10px 16px 16px;z-index:99;flex-direction:column;gap:3px;animation:slideDown .2s ease}
        .mobile-nav.open{display:flex}
        .mobile-bottom{display:none;position:fixed;bottom:0;left:0;right:0;background:rgba(10,20,34,.97);backdrop-filter:blur(16px);border-top:1px solid rgba(255,255,255,.07);padding:8px 0 max(8px,env(safe-area-inset-bottom));z-index:90}
        .mob-tabs{display:flex;justify-content:space-around}
        .mob-tab{display:flex;flex-direction:column;align-items:center;gap:2px;padding:6px 8px;border:none;background:none;cursor:pointer;min-width:0;flex:1}
        .mob-tab svg{width:20px;height:20px;stroke:rgba(255,255,255,.25);transition:stroke .15s}
        .mob-tab.on svg{stroke:#00b4c8}
        .mob-tab-lbl{font-size:9px;color:#3d4e62;font-family:'Instrument Sans',sans-serif;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:52px;transition:color .15s}
      ...
      <div className="d" dir={isRTL?'rtl':'ltr'}>
        ...
        <div className="mobile-topbar">
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <div style={{width:28,height:28,borderRadius:8,background:'linear-gradient(135deg,#028090,#00b4c8)',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Cabinet Grotesk,sans-serif',fontWeight:900,fontSize:11,color:'#fff'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:6}}>
            ...
            <div className="tlsw">
              {(Object.entries(FLAGS) as [Lang, typeof FLAGS[Lang]][]).map(([code, info]) => (
                <button key={code} className={`tlb${lang===code?' on':''}`} onClick={()=>setLang(code)}>
          ...
          <div className="topbar">
            <div>
              ...
              <div className="pg-sub">
                ...
                {tab==='questions'&&`${questions.length} questions · ${questions.length>5?dt('page_info'):'1 page'}${questionsDirty?` · ${dt('unsaved')}`:''}`}
                {tab==='qr'&&dt('gen_sub')}
                {tab==='settings'&&''}
              </div>
            </div>
          ...
          <div className="content">
            ...
            {tab==='overview'&&<>
              <div className="metrics">
                ...
                <div className="metric">
                  <div className="mlbl">{dt('week')}</div>
                  <div className="mval" style={{color:'#e8f0fa'}}>{week.length}</div>
                  {lastWk.length>0&&<div className="msub"><span className={week.length>=lastWk.length?'up':'dn'}>{week.length>=lastWk.length?'↑':'↓'} {Math.abs(week.length-lastWk.length)}</span></div>}
                </div>
                <div className="metric">
                  <div className="mlbl">{dt('score')}</div>
                  <div className="mval" style={{color:wAvg>0?SC(wAvg):'#2a3a52'}}>{wAvg>0?wAvg.toFixed(1):'—'}</div>
                  <div className="msub">/ 5</div>
                </div>
                {cats.length>0&&<div className="metric" style={{borderColor:'rgba(239,68,68,.18)'}}>
                  <div className="mlbl">{dt('weak')}</div>
                  <div className="mval" style={{color:SC(cats[0].avg),fontSize:24}}>{cats[0].avg.toFixed(1)}</div>
                  <div className="msub" style={{color:'#EF4444'}}>{cats[0].label}</div>
                </div>}
              </div>
              <div className="g2">
                <div className="card">
                  <div className="ch"><span className="ct">{dt('by_cat')}</span></div>
                  {cats.length===0?<div className="empty"><div className="empty-i">⭐</div><div className="empty-t">{dt('no_reviews')}</div></div>:
                    cats.map(c=>(
                ...
                <div className="card">
                  ...
                  {(()=>{
                    const days=Array.from({length:7},(_,i)=>{
                      ...
                      return{label:d.toLocaleDateString(lang==='ar'?'ar-MA':lang==='en'?'en-US':'fr-FR',{weekday:'short'}).slice(0,3),count:ss.length,isToday:d.toDateString()===new Date().toDateString()}
                    })
                    const mx=Math.max(...days.map(d=>d.count),1)
                    return<>
                      <div className="mc">{days.map((d,i)=><div key={i} className={`mb${d.isToday?' td':''}`} style={{height:`${Math.max((d.count/mx)*100,5)}%`}}/>)}</div>
              ...
              <div className="card">
                ...
                {submissions.length===0?<div className="empty"><div className="empty-i">💬</div><div className="empty-t">{dt('no_reviews')}</div><div className="empty-s">{dt('no_rev_sub')}</div></div>:
                  submissions.slice(0,4).map(s=>(
                    <div key={s.id} className="rv">
                      ...
                      <div className="rv-meta">
                        <div className="rv-cats">{Object.entries(s.ratings||{}).map(([id,v])=>{const cat=form?.categories.find(c=>c.id===id);return<span key={id} className="rv-tag">{cat?.label_fr||id}: {v as number}/5</span>})}</div>
                        {s.comment&&<div className="rv-cmt">&quot;{s.comment}&quot;</div>}
                      </div>
                      <div className="rv-time">{new Date(s.created_at).toLocaleDateString(lang==='ar'?'ar-MA':lang==='en'?'en-US':'fr-FR')}</div>
                    </div>
                  ))
                }
              </div>
            ...
            {tab==='questions'&&<>
              <div className="card">
                <div className="ch">
                  <span className="ct">{dt('my_q')}</span>
                  <div style={{display:'flex',alignItems:'center',gap:7}}>
                    ...
                    {questions.length>5&&<span style={{fontSize:9,color:'#7dd8e0',padding:'2px 7px',background:'rgba(0,180,200,.06)',border:'1px solid rgba(0,180,200,.14)',borderRadius:20}}>{dt('page_info')}</span>}
                  </div>
                </div>
                <div style={{height:3,background:'rgba(255,255,255,.05)',borderRadius:2,marginBottom:12,overflow:'hidden'}}>
                  <div style={{height:'100%',borderRadius:2,background:questions.length>=10?'#EF4444':questions.length>=5?'#10B981':'#F59E0B',width:`${(questions.length/10)*100}%`,transition:'width .3s ease'}}/>
                </div>
                {questions.map((q,idx)=>(
                  <div key={q.id} className="q-row">
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:12.5,fontWeight:600,color:'#e8f0fa',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{q.label_fr}</div>
                      <div style={{fontSize:9.5,color:'#3d4e62',marginTop:1}}>AR: {q.label_ar||'—'}</div>
                    </div>
                  ...
                  <div style={{marginTop:7}}>
                    {!showAddQ?(
                      <button onClick={()=>setShowAddQ(true)}
                        style={{width:'100%',padding:'9px',border:'1px dashed rgba(255,255,255,.1)',borderRadius:10,background:'transparent',color:'#4a5a72',cursor:'pointer',fontSize:12,fontFamily:'inherit',display:'flex',alignItems:'center',justifyContent:'center',gap:5,transition:'all .15s'}}
                      ...
                      <div style={{background:'#070f1d',border:'1px solid rgba(0,180,200,.2)',borderRadius:11,padding:12,animation:'fadeUp .2s ease'}}>
                        ...
                        {[{k:'fr',l:dt('fr_label')},{k:'ar',l:dt('ar_label')},{k:'en',l:dt('en_label')}].map(f=>(
                          <input key={f.k} placeholder={f.l} value={newQ[f.k as keyof typeof newQ]}
                            onChange={e=>setNewQ(p=>({...p,[f.k]:e.target.value}))}
                            style={{width:'100%',padding:'8px 11px',background:'#0a1525',border:'1.5px solid rgba(255,255,255,.08)',borderRadius:8,fontSize:12,color:'#e8f0fa',fontFamily:'inherit',outline:'none',marginBottom:6,display:'block'}}/>
                        ))}
              ...
              {questionsDirty&&(
                <div className="save-bar">
                  <div>
                    <div style={{fontSize:12.5,fontWeight:700,color:'#F59E0B',display:'flex',alignItems:'center',gap:5}}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                      {dt('unsaved')}
                    </div>
                    <div style={{fontSize:10,color:'#4a5a72',marginTop:2}}>Appliquez pour que vos clients voient les changements</div>
                  </div>
                  <div style={{display:'flex',gap:7,flexShrink:0}}>
                    ...
                    <button onClick={saveQ} disabled={questionsSaving}
                      style={{padding:'8px 18px',background:questionsSaving?'#028090':'linear-gradient(135deg,#028090,#00a8bc)',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:700,cursor:questionsSaving?'not-allowed':'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',gap:5}}>
            ...
            {tab==='qr'&&<>
              <div className="card">
                <div className="ch"><span className="ct">{dt('feedback_link')}</span></div>
                <div className="url-box">{formUrl}</div>
                <div style={{display:'flex',gap:7,flexWrap:'wrap'}}>
                  <button className="qb qbp" onClick={()=>navigator.clipboard.writeText(formUrl)}>
                  ...
                  <a href={formUrl} target="_blank" rel="noopener noreferrer" className="qb qbg" style={{textDecoration:'none'}}>
              ...
              <div className="card">
                ...
                {!qrGenerated?(
                  <div style={{textAlign:'center',padding:'28px 0'}}>
                    ...
                    <div style={{fontSize:13,color:'#4a5a72',marginBottom:6}}>{dt('gen_qr')}</div>
                    <div style={{fontSize:11,color:'#2a3a52',marginBottom:20}}>{dt('gen_sub')}</div>
                    <button className="qb qbp" onClick={generateQR} disabled={qrLoading} style={{padding:'11px 24px',fontSize:13}}>
                      {qrLoading?<><div style={{width:14,height:14,border:'2px solid rgba(255,255,255,.3)',borderTopColor:'#fff',borderRadius:'50%',animation:'spin .7s linear infinite'}}/>{dt('generating')}</>:<>📱 {dt('gen_qr')}</>}
                    </button>
                  </div>
                ):(
                  <div style={{display:'flex',gap:20,alignItems:'flex-start',flexWrap:'wrap'}}>
                    <div style={{flexShrink:0}}>
                      <div style={{background:'#fff',borderRadius:14,padding:14,display:'inline-block',boxShadow:'0 4px 20px rgba(0,0,0,.4)'}}>
                        <img key={qrKey} src={`${qrUrl}&v=${qrKey}`} alt="QR Code" width={180} height={180} style={{display:'block',borderRadius:3}}/>
                      </div>
                      <div style={{textAlign:'center',fontSize:9,color:'#2a3a52',marginTop:6}}>{business.slug}</div>
                    </div>
                    <div style={{flex:1,minWidth:180}}>
                      <div style={{fontFamily:'Cabinet Grotesk,sans-serif',fontSize:14,fontWeight:700,color:'#e8f0fa',marginBottom:5}}>{dt('qr_ready')}</div>
                      <div style={{fontSize:12,color:'#4a5a72',lineHeight:1.6,marginBottom:16}}>{dt('qr_desc')}</div>
                      <div style={{display:'flex',flexDirection:'column',gap:7}}>
                        <button className="qb qbp" onClick={downloadQR} style={{justifyContent:'flex-start'}}>
                        ...
                        <button className="qb qbw" onClick={()=>setShowRegenWarning(true)} style={{justifyContent:'flex-start'}}>
                          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-.53-8.94"/></svg>
                          {dt('regen')}
                        </button>
                      </div>
                      <div style={{marginTop:12,padding:'9px 11px',background:'rgba(0,180,200,.06)',border:'1px solid rgba(0,180,200,.14)',borderRadius:8,fontSize:10.5,color:'#7dd8e0',lineHeight:1.6}}>{dt('tip_qr')}</div>
                    </div>
                  </div>
                )}
              </div>
              <div className="card">
                <div className="ch"><span className="ct">{dt('stats')}</span></div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
                  {[{l:dt('total_scans'),v:submissions.length},{l:dt('this_week'),v:week.length},{l:dt('avg_score'),v:avg>0?avg.toFixed(1)+'/5':'—'}].map((m,i)=>(
                    <div key={i} style={{background:'#070f1d',borderRadius:8,padding:'11px',textAlign:'center',border:'1px solid rgba(255,255,255,.05)'}}>
                      <div style={{fontFamily:'Cabinet Grotesk,sans-serif',fontSize:20,fontWeight:900,color:'#e8f0fa',letterSpacing:-1}}>{m.v}</div>
                      <div style={{fontSize:9,color:'#2a3a52',marginTop:3}}>{m.l}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {tab==='settings'&&<>
              <div className="card">
                <div className="ch"><span className="ct">{dt('google_url')}</span></div>
                <p style={{fontSize:12.5,color:'#4a5a72',lineHeight:1.6,marginBottom:12}}>{dt('google_desc')}</p>
                <div className="sf">
                ...
                <div style={{display:'flex',alignItems:'center',gap:9}}>
                  <button className="sv-btn" onClick={saveGoogle} disabled={saving}>{saving?dt('saving'):dt('save')}</button>
                  {saved&&<div className="saved-b">{dt('saved')}</div>}
                </div>
              </div>
              <div className="card">
                <div className="ch"><span className="ct">{dt('info')}</span></div>
                {[{l:dt('name'),v:business.name},{l:dt('city'),v:business.city},{l:dt('sector_lbl'),v:business.sector},{l:dt('plan_lbl'),v:business.plan}].map((f,i)=>(
                  <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'9px 0',borderBottom:'1px solid rgba(255,255,255,.04)'}}>
                    <span style={{fontSize:10.5,color:'#3d4e62',textTransform:'uppercase',letterSpacing:'.5px',fontWeight:700}}>{f.l}</span>
                    <span style={{fontSize:12.5,color:'#8899b0'}}>{f.v}</span>
                  </div>
                ))}
                <p style={{fontSize:10,color:'#2a3a52',marginTop:10}}>{dt('contact_us')}</p>
              </div>
              <div className="card" style={{borderColor:'rgba(239,68,68,.14)'}}>
                <div className="ch"><span className="ct" style={{color:'#EF4444'}}>{dt('danger')}</span></div>
                <button onClick={logout} style={{padding:'8px 16px',background:'rgba(239,68,68,.08)',border:'1px solid rgba(239,68,68,.2)',borderRadius:8,color:'#EF4444',fontSize:12.5,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>
                  {dt('logout')}
                </button>
              </div>
            </>}
        ...
        <div className="overlay" onClick={()=>setShowRegenWarning(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div style={{display:'flex',alignItems:'center',gap:11,marginBottom:14}}>
              <div style={{width:40,height:40,borderRadius:11,background:'rgba(245,158,11,.12)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#F59E0B" strokeWidth="2" strokeLinecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
              </div>
              <div>
                <div style={{fontFamily:'Cabinet Grotesk,sans-serif',fontSize:15,fontWeight:800,color:'#e8f0fa',marginBottom:2}}>{dt('regen_warn')}</div>
              </div>
            </div>
            <div style={{fontSize:12.5,color:'#5a6a82',lineHeight:1.65,marginBottom:18,padding:'11px',background:'rgba(245,158,11,.06)',border:'1px solid rgba(245,158,11,.15)',borderRadius:9}}>{dt('regen_body')}</div>
            <div style={{display:'flex',gap:7}}>
              <button onClick={generateQR} disabled={qrLoading}
                style={{flex:1,padding:'10px',background:'#028090',color:'#fff',border:'none',borderRadius:9,fontSize:12.5,fontWeight:600,cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',justifyContent:'center',gap:5}}>
                {qrLoading?<><div style={{width:13,height:13,border:'2px solid rgba(255,255,255,.3)',borderTopColor:'#fff',borderRadius:'50%',animation:'spin .7s linear infinite'}}/>{dt('generating')}</>:<>{dt('regen_btn')}</>}
              </button>
              <button onClick={()=>setShowRegenWarning(false)} style={{padding:'10px 14px',background:'transparent',color:'#5a6a82',border:'1px solid rgba(255,255,255,.08)',borderRadius:9,fontSize:12.5,cursor:'pointer',fontFamily:'inherit'}}>{dt('cancel')}</button>
            </div>
          </div>
        </div>
      )}
