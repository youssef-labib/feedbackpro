...
const T: Record<string, Record<'fr' | 'ar' | 'en', string>> = {
  ...
  sub: { fr: 'Accedez a votre tableau de bord', ar: 'الوصول الى لوحة التحكم', en: 'Access your dashboard' },
  ...
  forgot: { fr: 'Mot de passe oublie ?', ar: 'نسيت كلمة المرور؟', en: 'Forgot password?' },
  submit: { fr: 'Se connecter', ar: 'تسجيل الدخول', en: 'Sign in' },
  loading: { fr: 'Connexion...', ar: 'جاري الدخول...', en: 'Signing in...' },
  no_acc: { fr: 'Pas de compte ?', ar: 'ليس لديك حساب؟', en: 'No account?' },
  register: { fr: "S'inscrire", ar: 'انشاء حساب', en: 'Sign up' },
  err_inv: { fr: 'Email ou mot de passe incorrect', ar: 'بيانات الدخول غير صحيحة', en: 'Invalid email or password' },
}
...
export default function LoginPage() {
  ...
  const isRTL = lang === 'ar'
  const t = (k: string) => T[k]?.[lang as 'fr' | 'ar' | 'en'] || k
  ...
  return (
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        @media(max-width:640px){
          .login-nav{padding:12px 16px!important;height:auto!important}
          .login-nav-right{width:100%;justify-content:space-between}
        }
      `}</style>
      ...
      <div style={{ minHeight: '100vh', background: '#07101f', display: 'flex', flexDirection: 'column' }} dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <nav className="login-nav" style={{ padding: '0 16px', minHeight: 58, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, borderBottom: '1px solid rgba(255,255,255,.06)', position: 'relative', zIndex: 10, flexShrink: 0, flexWrap: 'wrap' }}>
          ...
          <div className="login-nav-right" style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            <FlagLangSelector lang={lang} setLang={setLang} options={['fr', 'ar', 'en']} />
        ...
        <main style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 16px', position: 'relative', zIndex: 1 }}>
          <div style={{ width: '100%', maxWidth: 420, background: '#0d1927', border: '1px solid rgba(255,255,255,.08)', borderRadius: 20, padding: 'clamp(22px,5vw,32px) clamp(18px,5vw,28px)', boxShadow: '0 24px 64px rgba(0,0,0,.5)', animation: 'fadeUp .5s ease' }}>
            <div style={{ textAlign: 'center', marginBottom: 26 }}>
              ...
              <div style={{ fontFamily: 'Cabinet Grotesk,sans-serif', fontSize: 22, fontWeight: 900, color: '#e8f0fa', letterSpacing: -0.5, marginBottom: 4 }}>{t('title')}</div>
              <div style={{ fontSize: 13, color: '#4a5a72' }}>{t('sub')}</div>
            </div>

            <form onSubmit={handleLogin}>
              <div style={{ marginBottom: 14 }}>
                <label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: '#5a6a82', textTransform: 'uppercase', letterSpacing: 0.7, marginBottom: 6 }}>{t('email')}</label>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                  <label style={{ fontSize: 10, fontWeight: 700, color: '#5a6a82', textTransform: 'uppercase', letterSpacing: 0.7 }}>{t('pass')}</label>
